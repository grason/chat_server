		  ...
		  else 
		  {
			...
			send(i, line, strlen(line), 0);
			  ...
			  if(token != NULL)
			  {
			      ...
			      // Find the group that was requested...
			      for(vector<group*>::iterator group = groups.begin(); group != groups.end(); ++group)
			      {
				  // We have found the correct group...
				  if(!strcmp((*group)->group_name, groupname))
				  {
				      // Set a reference to all the members in said group...
				      vector<client*> cls = (*group)->members;
				      for(vector<client*>::iterator client = cls.begin(); client != cls.end(); ++client)
				      {
					  // If the requesting client is in the group...
					  if((*client)->socket == i)
					  {
			      ...
			      // If the client was in that group...
			      if(is_in_group)
			      {
				  int count = 0;
				  token = strtok(NULL, " ");
